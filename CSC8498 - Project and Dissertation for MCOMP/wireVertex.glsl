...
void main(void) {
	...
	float[5] newPos = mulVertex(projFloats4D, worldPos);
	for(int i=0; i<4; ++i)
		newPos[i] /= newPos[4];
	float[5] outPos = mulVertex(vpMat, newPos);
	gl_Position.x = outPos[0];
	gl_Position.y = outPos[1];
	gl_Position.z = outPos[2];
	gl_Position.w = outPos[4];

	OUT.worldPos.x = outPos[0];
	OUT.worldPos.y = outPos[1];
	OUT.worldPos.z = outPos[2];
	OUT.worldPos.w = outPos[3];

	float factor = (worldPos[3] + 1) / 2;  //-1 -> 1
	OUT.colour = colour * factor;
